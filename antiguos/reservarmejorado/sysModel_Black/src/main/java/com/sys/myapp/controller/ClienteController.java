...
public class ClienteController {
	...
	@GetMapping(value="/{clienteId}/cliente_borrar") 
    public String borrar_GET(Model model,@PathVariable Integer clienteId){        
		Cliente automod=cliService.findById(clienteId); 
        model.addAttribute("cliente", automod);
        return "/Cliente/Borrar";   
    }
	
    @PostMapping(value="/{clienteId}/cliente_borrar") 
    public String borrar_POST(Cliente cliente,@PathVariable Integer clienteId){          
        //antes de borrar busca en los hijos Reserva para lanzar error
    	ArrayList<Integer> listado1= (ArrayList<Integer>) cliService.clienteID_Reserva();
		for (Integer autoid: listado1  ) {
		if (autoid==clienteId)
			return "redirect:/Cliente/cliente_errorborrar";
		}    	
		 	//tendria q borrar en cascada con persona
    	//si esta todo ok, uso el id  recibido como clave del dicc pa borrar
		cliService.delete(clienteId);   
		perService.delete(clienteId);
        return "redirect:/Cliente/cliente_listar";   
    }
    
    @GetMapping(value="/Cliente/cliente_errorborrar")
    public String aerrorborrar_GET(){ 
		return "/Cliente/ErrorBorrar";   
    }    
