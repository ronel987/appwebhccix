...
public interface HabitacionRepository extends JpaRepository<Habitacion,Integer> {
  ...
  // con el checkout_al pagar: desocupa fisicamente: debe ser para una fecha_salida
    ...
    public abstract boolean desocupar (Integer habitaid, Date fecha);  //de java.sql.Date
                                                                          
    //con la reserva(ocupa) y con checkin : actualiza la fecha_ingresa: debe ser para una fecha_ingresa:en mysql si funca
    @Query(value="update habita_calendario hc join calendario c on hc.idcalendario=c.idcalendario set hc.estado='OCUPADA' where hc.idhabitacion=?1 and c.fecha like '%\"+ fecha + \"%'",nativeQuery=true)	
