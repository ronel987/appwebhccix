...
public interface HabitacionRepository extends JpaRepository<Habitacion,Integer> {
  ...
  //lista de Habitaciones disponibles por fecha_ingresa:si retorna un Set<Habitacion>
  ...
  // con el checkout_al pagar: desocupa fisicamente: debe ser para una fecha_salida
    ...
    //con la reserva(ocupa) y con checkin : actualiza la fecha_ingresa: debe ser para una fecha_ingresa
    @Query(value="update habita_calendario hc join calendario c on hc.idcalendario=c.idcalendario set estado='OCUPADA' where hc.idhabitacion=?1 and c.fecha=?2",nativeQuery=true)	
    public abstract boolean ocupar (Integer habitaid,Date fecha);
}
