...
public interface HabitacionRepository extends JpaRepository<Habitacion,Integer> {
  ...
  //lista de Habitaciones por piso:
    @Query(value="select * from habitacion where piso like '%\"+ piso + \"%' order by idhabitacion",nativeQuery=true)	    
    public abstract Collection<Habitacion> habitacionesxpiso(String piso);
  //lista de Habitaciones disponibles por piso:
    @Query(value="select * from habitacion where piso like '%\"+ piso + \"%' and estado='Disponible' order by idhabitacion",nativeQuery=true)	    
