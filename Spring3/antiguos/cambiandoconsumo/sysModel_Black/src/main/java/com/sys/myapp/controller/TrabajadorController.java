...
public class TrabajadorController {
    ...
    public String registrar_POST(Trabajador trabajador,RedirectAttributes flash){        //tra llega cargado al servidor  
				//no podes duplicar login:falta
		
		perService.insert(trabajador.getPersona());
		Integer idpersona= perService.obtenerultimo();
		trabajador.setIdpersona(idpersona);     //ope en memoria:para q tenga el mismo id del padre
		traService.insert(trabajador);
		flash.addFlashAttribute("success","Trabajador Guardado Exitosamente");
        return "redirect:/Traba/traba_registrar";   
    }    
    ...
    public String editar_GET(Model model,@PathVariable Integer trabajadorId,Map map,HttpSession session){        
		Optional <Trabajador> automod=traService.findById(trabajadorId); // esta cargado pa editarlo
	...
	@RequestMapping(value="/{trabajadorId}/traba_editar",method=RequestMethod.POST) 
    public String editar_POST(Trabajador trabajador ,@PathVariable Integer trabajadorId){  		
				
		Persona persona= trabajador.getPersona();        
		persona.setIdpersona(trabajadorId);
		perService.update(persona);	
		trabajador.setIdpersona(trabajadorId);
